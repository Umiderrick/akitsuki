package im.vinci.core.id.storer;

import im.vinci.core.id.SequenceStorer;
import im.vinci.core.id.StoreSequenceException;
...
public class FileSequenceStorer implements SequenceStorer {
	...
	/*	URL url = this.getClassLoader().getResource(this.filePath);
		if (url == null) {
		...
		return url.getFile();*/
		return this.filePath;//此处为消除异常所添加，若要使用此方法，需先进行测试
	}
